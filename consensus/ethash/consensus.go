...
var (
	...
	IMNBlockReward                = big.NewInt(0)     // Block reward in wei for IMN
...
func (ethash *Ethash) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header, uncle bool, seal bool, unixNow int64) error {
	...
	if imnminer.IsPoW() && header.Time <= parent.Time {
	...
	if !imnminer.IsPoW() && !imnminer.VerifyBlockSig(header.Number, header.Coinbase, header.MinerNodeId, header.Root, header.MinerNodeSig) {
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) error {
	...
	if imnminer.IsPoW() {
		...
		blockReward = IMNBlockReward
		coinbase, rewards, err := imnminer.CalculateRewards(
