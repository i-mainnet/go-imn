...
func SetupGenesisBlockWithOverride(db ethdb.Database, genesis *Genesis, overrideArrowGlacier, overrideTerminalTotalDifficulty *big.Int) (*params.ChainConfig, common.Hash, error) {
	...
	if genesis == nil && !(stored == params.MainnetGenesisHash || stored == params.ImnMainnetGenesisHash) {
...
func (g *Genesis) configOrDefault(ghash common.Hash) *params.ChainConfig {
	switch {
	case ghash == params.ImnMainnetGenesisHash:
		return params.ImnMainnetChainConfig
	case ghash == params.ImnTestnetGenesisHash:
		return params.ImnTestnetChainConfig
	case g != nil:
		return g.Config
...
func loadDefaultGenesisFile() (*Genesis, error) {
	genesis := new(Genesis)
	if _, err := os.Stat(params.ImnGenesisFile); err == nil {
		log.Info("Loading found genesis file", "name", params.ImnGenesisFile)
		file, err := os.Open(params.ImnGenesisFile)
		if err != nil {
			return nil, err
		}
		defer file.Close()
