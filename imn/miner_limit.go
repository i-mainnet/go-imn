...
func (ma *imnAdmin) collectMinerStates(height *big.Int) []*imnapi.IMNMinerStatus {
	var (
		ctx context.Context
	...
	var states []*imnapi.IMNMinerStatus
	for _, n := range e.nodes {
		if s, ok := miningPeers.Load(n.Name); ok {
			if status, ok := s.(*imnapi.IMNMinerStatus); ok {
...
func getCoinbaseEnodeCache(ctx context.Context, height *big.Int, gov *metclient.RemoteContract) (*coinbaseEnodeEntry, error) {
	...
	for i := int64(1); i <= count.Int64(); i++ {
		...
		e.nodes = append(e.nodes, &imnNode{
			Name:  string(name),
			Enode: string(enode), // note that this is not in hex unlike imnAdmin
			Id:    idv4,
			Addr:  addr,
		})
...
func (ma *imnAdmin) verifyMinerLimit(ctx context.Context, height *big.Int, gov *metclient.RemoteContract, coinbase *common.Address, enode []byte) (bool, error) {
	// parent block number
...
func (ma *imnAdmin) isEligibleMiner(height *big.Int) (bool, error) {
	var (
		ctx   context.Context
		enode []byte
		gov   *metclient.RemoteContract
	...
	if err != nil {
		return false, imnminer.ErrNotInitialized
	}
...
// beyond modifiedBlock + 1.
// Not enforced if member count <= 2.
func (ma *imnAdmin) nextMinerCandidates(height *big.Int) ([]*imnNode, error) {
	var (
		ctx context.Context
		gov *metclient.RemoteContract
		err error
...
func (ma *imnAdmin) electNextMiner(height *big.Int) error {
	tstart := time.Now()
	enode, err := hex.DecodeString(ma.self.Enode)
	if err != nil {
	...
	closeEnough := int64(2)
	peersMap := map[string]*imnapi.IMNMinerStatus{}
	if len(peers) > 0 {
		for _, p := range peers {
