...
// newWorkLoopEx is IMN's standalone goroutine to submit new mining work upon received events.
...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	blockInterval, _, blockGasLimit, baseFeeMaxChangeRate, gasTargetPercentage, err := imnminer.GetBlockBuildParameters(parent.Number())
	// Sanity check the timestamp correctness, recap the timestamp
	// to parent+1 if the mutation is allowed.
	timestamp := genParams.timestamp
	if imnminer.IsPoW() && parent.Time() >= timestamp {
...
func (w *worker) refreshPending(locked bool) {
	...
	blockInterval, _, blockGasLimit, baseFeeMaxChangeRate, gasTargetPercentage, _ := imnminer.GetBlockBuildParameters(parent.Number())
	num := parent.Number()
	num.Add(num, common.Big1)
	header := &types.Header{
		ParentHash: parent.Hash(),
