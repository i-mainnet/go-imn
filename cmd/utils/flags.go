...
var (
	...
	ImnTestnetFlag = cli.BoolFlag{
		Name:  "imn-testnet",
		Usage: "IMN test network: pre-configured imn test network",
	}
	...
	ConsensusMethodFlag = cli.IntFlag{
		...
		Usage: "IMN consensus method (integer, 1=PoW, 2=PoA, 3=ETCD, 4=PBFT)",
		Value: 2,
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	urls := params.ImnMainnetBootnodes
	switch {
	case ctx.GlobalIsSet(BootnodesFlag.Name):
		urls = SplitAndTrim(ctx.GlobalString(BootnodesFlag.Name))
	case ctx.GlobalBool(ImnTestnetFlag.Name):
		urls = params.ImnTestnetBootnodes
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, ImnTestnetFlag, DeveloperFlag, RopstenFlag, RinkebyFlag, GoerliFlag, SepoliaFlag)
	...
	case ctx.GlobalBool(MainnetFlag.Name):
		...
		SetDNSDiscoveryDefaults(cfg, params.ImnMainnetGenesisHash)
	case ctx.GlobalBool(ImnTestnetFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 8283
		}
		cfg.Genesis = core.DefaultTestnetGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.ImnTestnetGenesisHash)
	case ctx.GlobalBool(RopstenFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 3
		}
...
// SetIMNConfig applies imn related command line flags to the config.
func SetIMNConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	params.ImnGenesisFile = filepath.Join(ctx.GlobalString(DataDirFlag.Name), "genesis.json")
}
