...
const (
	clientIdentifier = "gimn" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-imn command line interface")
...
func prepare(ctx *cli.Context) {
	...
	switch {
	case ctx.GlobalIsSet(utils.ImnTestnetFlag.Name):
		log.Info("Starting Geth on IMN testnet...")
	...
	if ctx.GlobalString(utils.SyncModeFlag.Name) != "light" && !ctx.GlobalIsSet(utils.CacheFlag.Name) && !ctx.GlobalIsSet(utils.NetworkIdFlag.Name) {
		...
		if !ctx.GlobalIsSet(utils.ImnTestnetFlag.Name) &&
